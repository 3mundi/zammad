# Copyright (C) 2012-2018 Zammad Foundation, http://zammad-foundation.org/

class Transaction::Rocket

=begin

  backend = Transaction::Rocket.new(
    object: 'Ticket',
    type: 'update',
    object_id: 123,
...
=end
  ...
  def perform
    ...
    return if !Setting.get('rocket_integration')

    config = Setting.get('rocket_config')
    return if !config
    ...
    result = NotificationFactory::Rocket.template(
      template: template,
    ...
    config['items'].each do |local_config|
      ...
      cache_key = "rocket::backend::#{@item[:type]}::#{ticket.id}::#{md5_webhook}"
      if sent_value
  ...
  class Transaction::Rocket::Client
